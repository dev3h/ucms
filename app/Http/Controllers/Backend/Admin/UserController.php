...
use App\Mail\CreateNewAccountMail;
...
class UserController extends Controller
{
    ...
    public function store(UserRequest $request)
    {
        ...
        try {
            ...
            Mail::to($dataValidate['email'])->send(new CreateNewAccountMail($data_send));
    ...
    {
        return Excel::download(new UsersExport, 'template-user.csv');
    }
    ...
    public function importCsv(ImportUserRequest $request)
    {
        try {
            ...
            foreach ($data as $item) {
                ...
                $account = User::create(Arr::except($accountData, ['real_password']));
                $loginUrl = route('user.login.form');
                $data_send = [
                    'user' => [
                        'name' => $accountData['name'],
                        'email' => $accountData['email'],
                ...
                Mail::to($accountData['email'])->send(new CreateNewAccountMail($data_send));
            }

            DB::commit();
            return $this->sendSuccessResponse(null, __('Imported successfully'));
        } catch (\Throwable $e) {
            DB::rollBack();
            return $this->sendErrorResponse(__('Something went wrong'), $e->getMessage());
        }
